...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === 'CLEAR_STATE') {
        ...
        const { command, target, text, requestId, sourceClientId, format } = request.data;
        let result = {};

        try {
            if (command === 'get_web_content') {
                let content = '';
                if (format === 'html') {
            ...
            } else if (command === 'click_element_by_selector') {
                const element = document.querySelector(target);
                if (!element) {
                    throw new Error(`未能在页面上找到目标选择器为 '${target}' 的元素。`);
                }
                element.focus();
                ...
                result = { status: 'success', message: `成功点击了选择器为 '${target}' 的元素。` };

            } else {
                // Handle existing commands that use vcp-id
